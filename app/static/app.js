...
const samplerFrequencyPenalty = document.querySelector(
	"#sampler-frequency-penalty",
);
const samplerPresencePenalty = document.querySelector(
	"#sampler-presence-penalty",
);
...
const additionalSystemPromptInput = document.querySelector(
	"#additional-system-prompt",
);
const companionConversationEl = document.querySelector(
...
const companionContinueButton = document.querySelector(
	"#companion-continue-btn",
);
...
const companionMemoryEnabledInput = document.querySelector(
	"#companion-memory-enabled",
);
const companionSystemPromptInput = document.querySelector(
...
const companionSamplerTemperature = document.querySelector(
	"#companion-sampler-temperature",
);
...
const companionSamplerMaxTokens = document.querySelector(
	"#companion-sampler-max-tokens",
);
const companionSamplerResetButton = document.querySelector(
	"#companion-sampler-reset",
);
...
function applyStatus(status) {
	...
	if (
		state.pendingTtsPresetId &&
		status.tts_runtime?.selected_preset_id === state.pendingTtsPresetId
	) {
		state.pendingTtsPresetId = null;
	}
	if (
		state.pendingTtsVoiceId &&
		status.tts_runtime?.selected_voice === state.pendingTtsVoiceId
	) {
...
function renderTtsRuntime(runtime) {
	...
	const selectedPresetId =
		state.pendingTtsPresetId || runtime?.selected_preset_id;
	...
	ttsVoiceEl.disabled =
		state.ttsBusy || !supportsVoiceSelection || voices.length === 0;
...
function handleEvent(event) {
	...
	if (event.type === "providers.status") {
		...
		const nodeKey =
			mode === "companion" ? "companionAssistantNode" : "assistantNode";
		if (!state[nodeKey]) {
...
function updateSendState() {
	...
	if (!state.providersReady) {
		textInput.placeholder =
			"A required provider is not ready. Check the status cards above.";
	} else {
		textInput.placeholder =
			"Type a test utterance while the audio path is being wired...";
	}
}
...
async function toggleMic() {
	...
	processor.onaudioprocess = (event) =>
		handleAudioProcess(event.inputBuffer.getChannelData(0), context.sampleRate);
			...
			payload: {
				...
				system_prompt:
					state.view === "companion"
						? companionSystemPromptInput.value
...
function renderCharacter() {
	...
	if (card && card.name) {
		...
		characterDescEl.textContent =
			card.description || card.personality || "No description.";
		const parts = [];
		...
		if (card.system_prompt)
			parts.push(`Card system prompt:\n${card.system_prompt}`);
		characterFullEl.textContent =
...
async function resetCompanionSampler() {
	try {
		const result = await patchJson("/api/settings", {
			companion: { llm_overrides: {} },
		});
		applySettings(result);
	} catch (error) {
	...
	try {
		...
		while (state.audioQueue.length > 0) {
			...
			const leadMs = Math.max(
				0,
				Math.round((state.nextAudioTime - now) * 1000),
			);
			const startAt = Math.max(
				now + state.playbackLeadTime,
				state.nextAudioTime,
			);
			...
			source.addEventListener("ended", () => {
				state.scheduledSources = state.scheduledSources.filter(
...
function sendCompanionPromptUpdate() {
	...
	state.ws.send(
		JSON.stringify({
			...
			payload: {
				mode: "companion",
				system_prompt: companionSystemPromptInput.value,
			},
		}),
	);
}
...
async function saveMemory() {
	try {
		const payload = await putJson("/api/companion/memory", {
			text: companionMemoryInput.value,
		});
		companionMemoryInput.value = payload.text || "";
	...
	if (state.ws && state.ws.readyState === WebSocket.OPEN) {
		state.ws.send(
			JSON.stringify({
				...
				payload: { mode: state.view === "companion" ? "companion" : "chat" },
			}),
		);
	} else {
		conversationForMode(
			state.view === "companion" ? "companion" : "chat",
		).innerHTML = "";
		latencyEl.innerHTML = "";
	}
});
...
ttsPresetEl.addEventListener("change", () => {
	...
	state.pendingTtsVoiceId = null;
	withTtsBusy(() =>
		postJson("/api/tts/select", { preset_id: ttsPresetEl.value }),
	);
});
...
companionSystemPromptInput.addEventListener(
	"change",
	sendCompanionPromptUpdate,
);
...
const samplerInputs = [
	...
	samplerTopP,
	samplerMinP,
	samplerRepeatPenalty,
	samplerFrequencyPenalty,
	samplerPresencePenalty,
	samplerMaxTokens,
];
...
for (const input of [
	companionSamplerTemperature,
	companionSamplerTopP,
	companionSamplerMinP,
	companionSamplerMaxTokens,
]) {
...
if (sidebarToggle && workspace) {
	sidebarToggle.addEventListener("click", () => {
		workspace.classList.toggle("sidebar-open");
		sidebarToggle.textContent = workspace.classList.contains("sidebar-open")
			? "Hide Metrics ▾"
			: "Metrics ▸";
	});
}

/* ===== Responsive: companion sub-tabs ===== */
const companionSubtabs = document.querySelectorAll(".companion-subtab");
const companionWorkspace = document.querySelector(".companion-workspace");
for (const btn of companionSubtabs) {
	btn.addEventListener("click", () => {
		for (const b of companionSubtabs) b.classList.remove("active");
		btn.classList.add("active");
		if (companionWorkspace) {
			companionWorkspace.dataset.activeSubtab = btn.dataset.subtab;
		}
	});
}

/* ===== Responsive: auto-collapse panels on mobile ===== */
