...
class MockASRProvider(ASRProvider):
    ...
    async def transcribe_audio(self, pcm_s16le: bytes, sample_rate: int, progress=None) -> AsyncIterator[TranscriptEvent]:
        if progress:
...
class MockLLMProvider(LLMProvider):
    ...
    async def stream_response(self, messages: list[dict[str, str]]) -> AsyncIterator[str]:
        ...
        response = (
            "I heard: "
            + user_text
            + ". This is the harness running in mock mode, with provider boundaries and latency events active."
        )
...
class MockTTSProvider(TTSProvider):
    ...
    async def stream_audio(self, text: str) -> AsyncIterator[AudioChunk]:
        ...
        yield AudioChunk(data=make_tone_wav(frequency=frequency), mime_type="audio/wav", final=True)
        await asyncio.sleep(self.chunk_delay)

    async def stream_audio_with_progress(self, text: str, progress=None) -> AsyncIterator[AudioChunk]:
        if progress:
            await progress("tts.progress", {"stage": "mock", "message": "Mock TTS generating tone."})
